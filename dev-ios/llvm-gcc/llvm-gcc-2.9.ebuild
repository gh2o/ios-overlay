...
src_prepare() {
	...
	cd "${S}"
	epatch "${FILESDIR}/gcc-darwin-mempcpy.patch"
	epatch "${FILESDIR}/gcc-llvm-isystem.patch"
	epatch "${FILESDIR}/gcc-arm-nov5.patch"
...
src_configure() {
	...
	ECONF_SOURCE="${S}" econf --prefix="${EPREFIX}"/usr \
		$(use_enable multilib) \
		--enable-llvm="${EPREFIX}"/usr --enable-languages=c,c++${EXTRALANGS} \
		$(use prefix && echo "--with-local-prefix=${EPREFIX}/usr") \
		--target=${CTARGET} \
		--with-sysroot=/usr/${CTARGET} --with-build-sysroot=/usr/${CTARGET} \
		--with-as=/usr/bin/${CTARGET}-as \
		--with-ld=/usr/bin/${CTARGET}-ld \
